...
const getCardNameMapping = (cardName: string): string => {
...
const loadTarotData = (): Record<string, any> => {
  try {
...
// Кэшируем данные
let tarotDataCache: Record<string, any> | null = null;

// Функция для получения данных карты из JSON
const getCardDataFromJSON = (cardName: string): any => {
  if (!tarotDataCache) {
...
export const getCardDescriptionByName = (cardName: string, category: string): string => {
  ...
  return fallbackDescriptions[categoryKey as keyof typeof fallbackDescriptions] || `Карта "${cardName}" символизирует надежду и вдохновение.`;
};

// Функция для обогащения карты подробными описаниями
export const enrichCardWithDetailedDescription = (card: TarotCard, category?: string): TarotCard => {
