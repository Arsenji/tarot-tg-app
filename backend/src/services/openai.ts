...
class OpenAIService {
  ...
  private buildReadingPrompt(request: ReadingRequest): string {
    ...
    if (readingType === 'yesno') {
      ...
      prompt += '\n\n';
      prompt += 'ВАЖНО: Первая строка ответа — строго одно слово "Да" или "Нет" (без точки и пояснений).\n';
      prompt += 'Затем, с новой строки, дай развёрнутое, тёплое и полезное толкование объёмом 5–7 предложений (2 небольших абзаца), которое:\n';
      prompt += '1) объясняет, почему именно эта карта (с учётом того, перевёрнута она или нет) даёт такой ответ, связывая это с вопросом человека;\n';
      prompt += '2) раскрывает нюансы, на что стоит обратить внимание, какие есть возможности или риски в этой ситуации;\n';
      prompt += '3) даёт конкретный практический совет, как действовать дальше;\n';
      prompt += '4) завершается поддерживающей, вдохновляющей мыслью, чтобы человек почувствовал опору.\n';
      prompt += 'Обращайся к человеку на "ты", пиши живым, эмпатичным и человечным языком, конкретно и по делу, без канцелярита, воды и повторов.\n';
      prompt += 'Не начинай толкование со слов "Да"/"Нет" повторно — они уже есть в первой строке.\n';
      ...
      prompt += 'Не используй символы форматирования (*, #, _) — только обычный текст.\n';
    ...
    if (readingType === 'three') {
      ...
      prompt = `Ты опытный таролог. Сделай расклад из трёх карт по позициям Прошлое — Настоящее — Будущее.\n\n`;
      ...
      cards.forEach((card) => {
        ...
        prompt += `- ${positionName}: ${getRussianCardName(card.name)}${card.isReversed ? ' (перевёрнутая)' : ''}\n`;
      });

      prompt += '\nСформируй ответ СТРОГО в таком формате (без markdown, без звёздочек и решёток):\n\n';
      prompt += 'Для твоего вопроса подойдёт расклад Прошлое - Настоящее - Будущее.\n\n';
      prompt += 'Карта <Название> — Прошлое\n<2-3 предложения о значении этой карты в этой позиции и в контексте вопроса>\n\n';
      prompt += 'Карта <Название> — Настоящее\n<2-3 предложения>\n\n';
      prompt += 'Карта <Название> — Будущее\n<2-3 предложения>\n\n';
      prompt += 'Расклад показывает: <итоговая связная интерпретация всего расклада в 3-5 предложениях>\n\n';
      prompt += 'ВАЖНО:\n';
      prompt += '- Подставь реальные названия выпавших карт и учитывай, перевёрнута ли карта.\n';
      prompt += '- Используй ТОЛЬКО русский язык и русские названия карт (например, "Звезда", "Башня", "Смерть"), никогда не пиши английские названия вроде "The Star".\n';
      prompt += '- Обращайся к человеку на "ты", пиши тепло, конкретно и по делу.\n';
      prompt += '- Не используй символы форматирования (*, #, _) — только обычный текст в указанной структуре.';
    } else {
  ...
  async getClarifyingAnswer(
    ...
    try {
      ...
      if (readingType === 'yesno') {
        prompt += 'ВАЖНО:\n';
        prompt += '- Первая строка ответа — строго одно слово "Да" или "Нет" (без точки и пояснений).\n';
        prompt += '- Затем, с новой строки, дай развёрнутое, тёплое и полезное толкование объёмом 5–7 предложений (2 небольших абзаца): объясни, почему карта даёт такой ответ именно в контексте уточняющего вопроса, раскрой нюансы и на что обратить внимание, дай конкретный практический совет и заверши поддерживающей мыслью.\n';
        prompt += '- Не начинай толкование со слов "Да"/"Нет" повторно — они уже есть в первой строке.\n';
        ...
        prompt += '- Не используй символы форматирования (*, #, _) — только обычный текст.\n';
        prompt += '- Пиши живо, эмпатично и по делу, обращаясь к человеку на "ты".';
      } else {
        prompt += 'ВАЖНО:\n';
