    ...
    const imagePath = getCardImagePath(randomCard, isReversed);
        ...
        try {
          if (interpretation.interpretation && interpretation.interpretation.trim().length > 0) {
            // Сохраняем только если есть подписка
            if (subscriptionStatus.hasSubscription || isAdmin) {
              logger.info('Attempting to save daily-advice reading (user has subscription)', {
...
router.post('/three-cards', async (req: any, res) => {
  try {
    ...
    // Отмечаем использование Three Cards для бесплатных пользователей
    if (!subscriptionStatus.hasSubscription && !isAdmin) {
    ...
    try {
      if (interpretation.interpretation && interpretation.interpretation.trim().length > 0) {
        // Сохраняем только если есть подписка
        if (subscriptionStatus.hasSubscription || isAdmin) {
          logger.info('Attempting to save three-cards reading (user has subscription)', {
            userId: req.user.userId,
            telegramId: userId,
  ...
  try {
    ...
    try {
      if (interpretationText && interpretationText.trim().length > 0) {
        // Сохраняем только если есть подписка
        if (subscriptionStatus.hasSubscription || isAdmin) {
          logger.info('Attempting to save yes-no reading (user has subscription)', {
            userId: req.user.userId,
            telegramId: userId,
            cardName: randomCard,
